...
export class PageHeader extends AppElement {
    ...
    render() {
        let logo = this.state.context?.theme === 'light'
            ? this.state.brand?.src
            : (this.state.brand?.srcDark === undefined ? this.state.brand?.src : this.state.brand?.srcDark);

        render(
            <header>
                <nav class={this.getClassNames(["navbar"], this.state.classList)} role="navigation" aria-label="main navigation">
                    <div class="navbar-brand">
                        <img class="navbar-item" src={logo} width="160" height="40" />
