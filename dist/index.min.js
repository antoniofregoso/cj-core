var k=class extends HTMLElement{#l={};#c=new Set;constructor(c={}){super(),this.state=this.initState(c)}initState(c,s){if(s!=null){let a=Object.assign({},c,s);return c!=null&&Object.keys(c).lenght!=0&&Object.keys(c).forEach(e=>{s[e]!=null&&(typeof s[e]=="string"||Array.isArray(s[e])?a[e]=s[e]:a[e]=Object.assign({},c[e],s[e]))}),a}else return c}attribute2CamelCase(c){let s=new RegExp("-([a-z])","g");return c.replace(s,(a,e)=>e.toUpperCase())}camelCase2attribute(c){return c.replace(new RegExp("-([a-z])","g"),(s,a)=>a.toUpperCase())}setState(c){this.state=this.initState(this.#l,c),this.render()}updateState(c){this.state=this.initState(this.state,c),this.render()}setAnimation(c){if(c==null)return"";{let s=` data-animation=${c.effect}`;return c.delay!=null&&(s+=` data-delay=${c.delay}`),c.speed!=null&&(s+=` data-speed=${c.speed}`),c.repeat!=null&&(s+=` data-repeat=${c.repeat}`),s}}cleanValue(c){return c??""}updateClassList(){this.state.classList&&this.classList.add(...this.state.classList)}getClasses(c=[],s){let a=[];s===void 0?a=c:a=[...c,...s];let e="";return a.length>0&&(e=`class="${a.toString().replaceAll(","," ")}"`),e}getBackground(){let c="";return this.state.backgroundImage?.url!=null?(c=`background-image: url('${this.state.backgroundImage.url}'); background-repeat: no-repeat; background-position: center; background-size: cover;`,this.state.backgroundImage?.fixed&&(c=`${c} background-attachment: fixed;`),this.state.backgroundImage?.filter&&(c=`${c} filter: ${this.state.backgroundImage?.filter};`)):c="",` style="${c}"`}getTitles(){let c="";return this.state!=null&&(c=`
            ...
            </div>`),c}handleEvent(c){if(c.type==="click"){this.eventName=this.state.buttons?.eventName??this.state.eventName??this.eventName;let s=new CustomEvent(this.eventName,{detail:{source:c.target.id},bubbles:!0,composed:!0});this.dispatchEvent(s)}}registerExtraEvents(){}addEvents(){let c=this.querySelectorAll("button");c.length>0&&c.forEach(s=>{s.addEventListener("click",this)})}#s(c){if(c!=null){let s="";return c.forEach(a=>{s+=`<${a.href!=null?"a":"button"} id="${a.id}" ${this.getClasses(["button"],a.classList)} ${a.href!=null?`href="${a.href}"`:""}>${a?.text[this.state.context.lang]}</${a.href!=null?"a":"button"}>`}),s}else return""}buttonsRender(c){return c!=null?`
                <p ${this.getClasses(["buttons","mt-4"],c.classList)}>
                    ${this.#s(c.buttons)}
                </p>
            `:""}addCleanup(c){return typeof c!="function"?()=>{}:(this.#c.add(c),()=>{this.#c.delete(c)})}runCleanups(){this.#c.forEach(c=>{try{c()}catch(s){console.error("Error during component cleanup",s)}}),this.#c.clear()}render(){console.error("Nothing to render")}connectedCallback(){this.render()}disconnectedCallback(){let c=this.querySelectorAll("button");c.length>0&&c.forEach(s=>{s.removeEventListener("click",this)}),this.runCleanups()}};customElements.define("app-element",k);var r2=class extends k{#l={events:{viewedElement:void 0,leavingApp:!1,leavedApp:!1},REABANDONMENT_THRESHOLD_MS:3*60*1e3,classList:[]};#c=[];constructor(c={},s=null){super(),this.data=c,this.template=s,this.scrollStopping={name:"",session:"",page:{start:Date.now(),end:0,time:0,leavingapp:0,views:0,req:{}},sections:{}};try{let a=document.querySelector("#app");a.innerHTML="",a.appendChild(this)}catch(a){console.error('The element with id "app" does not exist to insert the element "app-page".',a)}}#s=!1;#a=0;#e(){let c=document.getElementsByTagName("head")[0],s=document.createElement("meta");s.name="Cache-Control",s.content="no-cache, no-store, must-revalidate",c.appendChild(s);let a=document.createElement("meta");a.name="Pragma",a.content="no-cache",c.appendChild(a);let e=document.createElement("meta");e.name="Expires",e.content="0",c.appendChild(e)}#n(){let c=this.data.props,s=this.data.context,a=document.getElementsByTagName("head")[0];if(document.title!=null){document.title=c.title[s.lang];let e=document.querySelector("meta[name=description]");if(e===null){let g=document.createElement("meta");g.name="description",g.content=c.description[s.lang],a.appendChild(g)}else e.content=c.description[s.lang];let n=document.createElement("meta");n.setAttribute("property","og:title"),n.content=c.title[s.lang],a.appendChild(n);let o=document.createElement("meta");o.setAttribute("property","og:description"),o.content=c.description[s.lang],a.appendChild(o);let i=document.createElement("meta");i.setAttribute("property","og:type"),i.content=c.type,a.appendChild(i);let m=document.createElement("meta");m.setAttribute("property","og:image"),m.content=c.image,a.appendChild(m);let r=document.createElement("meta");r.name="twitter:card",r.content="summary_large_image",a.appendChild(r);let z=document.createElement("meta");z.name="twitter:title",z.content=c.title[s.lang],a.appendChild(z);let M=document.createElement("meta");M.name="twitter:description",M.content=c.title[s.lang],a.appendChild(M);let L=document.createElement("meta");L.name="twitter:image",L.content=c.image,a.appendChild(L);let C=document.createElement("link");C.setAttribute("rel","canonical"),C.setAttribute("href",c.canonical),a.appendChild(C)}}#f(){this.data.props?.classList?.length>0&&document.body.classList.add(...this.data.props.classList)}#r(c,s={}){return c!=null&&(c.context=s),c}setSectionViewed(c,s){return{[c]:{order:s.order,start:Date.now(),end:0,time:0,views:s.views+1}}}setPageQuit(c){return{start:c.start,end:Date.now(),time:Date.now()-c.start,leavingapp:c.leavingapp,views:c.views,req:c.req}}loadData(){if(this.data.props=this.initState(this.#l,this.data.props),this.data.props.id!=null){this.getAttribute("id")||this.setAttribute("id",this.data.props.id||`component-${Math.floor(Math.random()*100)}`),this.#n(),this.#f(),this.state.Cache===!1&&this.#e(),this.data.props.components.forEach(s=>{try{this.querySelector(`#${s.id}`).updateState(this.#r(s,this.data.context))}catch(a){console.error(`The Element with id ${s.id} does not exist or is not an object of type AppElement`,a)}});let c=document.querySelector(".pageloader");c?.classList.remove("is-active")}}async sendWebhook(c,s,a={},e=!0){let n={method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)},o=document.querySelector(".pageloader");o?.classList.add("is-active");try{let i=await fetch(c,n);if(!i.ok)throw new Error(`HTTP error! status: ${i.status}`);let m=await i.json();if(e==!0)try{m.context=a,this.data=m,this.loadData()}catch(r){console.error('Cannot load data to render "app-page" components',r)}else return m}catch(i){console.error("Error:",i)}}#m(c,s){let a=/id="([^"]+)"/g,e,n=[];for(;(e=a.exec(c))!==null;)n.push(e[1]);return n.filter(i=>s.includes(i))}#i(){if(this.#s&&Date.now()-this.#a<this.state.REABANDONMENT_THRESHOLD_MS)return;this.#s=!0,this.#a=Date.now();let c=new CustomEvent("leavedapp",{detail:{source:this.data.props.id},bubbles:!0,composed:!0});this.dispatchEvent(c)}#o(c){typeof c=="function"&&this.#c.push(c)}#t(){this.#c.forEach(c=>{try{c()}catch(s){console.error("Error during page render cleanup",s)}}),this.#c=[]}#z(){if(this.#t(),Array.isArray(this.data.props.events.trackViewed)){let c=this.#m(this.template,this.data.props.events.trackViewed),s=0;c.forEach(e=>{this.scrollStopping.sections[e]={order:s,start:0,end:0,time:0,views:0},s++});let a=new IntersectionObserver(e=>{e.forEach(n=>{this.scrollStopping;let o=n.target.id;if(n.isIntersecting){let i=new CustomEvent("viewedelement",{detail:{source:o},bubbles:!0,composed:!0});this.dispatchEvent(i)}})},{root:null,rootMargin:"0px",threshold:.25});this.data.props.events.trackViewed.forEach(e=>{let n=this.querySelector(`#${e}`);n&&a.observe(n)}),this.#o(()=>a.disconnect())}if(this.data.props?.events?.leavingapp===!0){let c=new CustomEvent("leavingapp",{detail:{source:this.data.props.id},bubbles:!0,composed:!0}),s=a=>{(a.clientY<=0||a.clientX<=0||a.clientX>=window.innerWidth||a.clientY>=window.innerHeight)&&this.dispatchEvent(c)};document.addEventListener("mouseleave",s),this.#o(()=>{document.removeEventListener("mouseleave",s)})}if(this.data.props?.events?.leavedapp===!0){let c=()=>{document.visibilityState==="hidden"&&this.#i()},s=()=>{this.#i()};document.addEventListener("visibilitychange",c),window.addEventListener("pagehide",s),this.#o(()=>{document.removeEventListener("visibilitychange",c),window.removeEventListener("pagehide",s)})}}#L(c){let s=new Set;function a(e){if(!(!e||typeof e!="object")){e.hasOwnProperty("eventName")&&s.add(e.eventName);for(let n in e)e.hasOwnProperty(n)&&typeof e[n]=="object"&&e[n]!==null&&a(e[n])}}return a(c),Array.from(s)}setEvents(c){let s=this.#L(this.data.props.components);s.push("user:select-lang","user:select-theme"),this.data.props.events.leavingapp===!0&&s.push("leavingapp"),this.data.props.events.leavedapp===!0&&s.push("leavedapp"),Array.isArray(this.data.props.events.trackViewed)&&this.data.props.events.trackViewed.length>0&&s.push("viewedelement"),s.forEach((a,e)=>{this.addEventListener(a,c)})}render(){this.template===null?console.error("No component template provided"):(this.innerHTML=this.template,this.data.props?.id!=null&&(this.loadData(this.data),this.#z()))}disconnectedCallback(){this.#t(),super.disconnectedCallback()}};customElements.define("app-page",r2);function i0(l){if(!l)return"";var c=l.toLowerCase().trim();return c=c.normalize("NFD").replace(/[\u0300-\u036f]/g,""),c=c.replace(/[^a-z0-9\s-]/g," ").trim(),c=c.replace(/[\s-]+/g,"%20"),c}function t0(l=32){let c="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",s=new Uint8Array(l);crypto.getRandomValues(s);let a="";for(let e=0;e<l;e++){let n=s[e]%c.length;a+=c[n]}return a}function r0(){let l=document.querySelectorAll("[data-animation]"),c={threshold:.1};var s=new IntersectionObserver(a=>{a.forEach(e=>{if(e.isIntersecting){a3(e.target);return}e.target.classList.forEach(n=>{n.startsWith("animate__")&&e.target.classList.remove(n)})})});l.forEach(a=>{s.observe(a)})}function a3(l){l.classList.add("animate__animated",`animate__${l.getAttribute("data-animation")}`),l.hasAttribute("data-delay")&&l.classList.add(`animate__delay-${l.getAttribute("data-delay")}`),l.hasAttribute("data-speed")&&l.classList.add(`animate__${l.getAttribute("data-speed")}`),l.hasAttribute("data-repeat")&&(l.getAttribute("data-repeat")==="infinite"?l.classList.add("animate__infinite"):l.classList.add(`animate__repeat-${l.getAttribute("data-repeat")}`))}function m0(){return navigator.languages!=null?navigator.languages[0].substring(0,2):navigator.language.substring(0,2)}function z0(l){l===void 0&&(l={});let s=["is-bottom-to-top","is-right-to-left","is-left-to-right"].find(a=>a==l.direction);document.body.innerHTML+=`<div class="pageloader ${l?.color!=null?l.color:"is-light"} ${s??""} is-active"><span class="title">${l.message!=null?l.message:"Loading..."}</span></div>`}var m2=class extends k{#l={brand:{name:"CustumerJourney.js",url:"https://customerjourney.ninja/"}};constructor(c={}){super(),this.state=this.initState(this.#l,c),this.getAttribute("id")||this.setAttribute("id",this.state.id||`component-${Math.floor(Math.random()*100)}`)}render(){this.innerHTML=`
                ...
                <div class="navbar-item has-dropdown">
                    ...
                    <div class="navbar-dropdown">                   
                        <a id="light-theme" class="navbar-item">
                            ${this.#c} ${this.state.themeValues.light.text[this.state.context?.lang]}
                        </a>
